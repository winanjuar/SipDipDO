...
-- deterministik-unik per baris dari md5(id) — heksadesimal uppercase (0-9A-F)
-- adalah subset alfanumerik. Baris baru menyusul dari generator repo
-- (buatKodeReferral), bukan dari SQL ini.
UPDATE "owners" SET "referral_code" = upper(substr(md5("id"::text), 1, 8)) WHERE "referral_code" IS NULL;--> statement-breakpoint
