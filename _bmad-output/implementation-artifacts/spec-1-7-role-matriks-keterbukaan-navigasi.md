...
## Spec Change Log (renegosiasi intent — keputusan owner di luar blok beku)

- **2026-09-21 — `/personal` dibuka untuk SEMUA owner (read-only).** Matriks I/O beku baris "GET /api/personal role lain → 403" dan "coo × /personal → redirect" DIGANTIKAN: prasyarat `/personal` = owner terautentikasi apa pun (`PRASYARAT_OWNER`), `GET /api/personal` 200 untuk coo/pemegang_saham/tanpa_saham (403 tetap untuk calon_owner). Alasan: semua owner (termasuk COO) perlu melihat data profil dirinya; MUTASI data tetap satu pintu via COO (FR-13, Story 1.8). Pintu Pesanan di Halaman Personal hanya tampil bagi yang belum pernah membeli. Nav: +Personal untuk coo & pemegang_saham. Blok `<frozen-after-approval>` tidak diubah — catatan ini yang mensupersesi.
- **2026-09-22 — `/pendaftar` terdaftar di registry keterbukaan & navigasi COO.** Registry beku "permukaan Epic 1 saja" dan daftar nav awal `coo` = "Antrian Beli, Dashboard, Audit Trail" DIGANTIKAN sebagian: `/pendaftar` (permukaan verifikasi Story 1.6) masuk `PERMUKAAN_PERAN` sebagai `PRASYARAT_COO` — non-COO dialihkan di batas server (pemegang_saham → `/dashboard`; tanpa_saham belum-beli → jalur transparansi flash-cookie; calon tetap gerbang calon 1.5 lebih dulu), dan item nav `Pendaftar` disisip setelah `Order` sebelum `Audit` (urutan coo = Dashboard, Personal, MoM, Order, Pendaftar, Audit — keputusan owner 2026-09-22; Sheet "Lainnya" mobile berisi Pendaftar, Audit). Alasan: temuan review adv#2/adv#3 — permukaan `/pendaftar` tidak tergerbangi registry di batas server dan hanya terjangkau URL manual, melanggar prinsip "sumber tunggal kebenaran keterbukaan" spec ini. Blok `<frozen-after-approval>` tidak diubah — catatan ini yang mensupersesi.
