...
- **2026-09-18 (re-negotiasi owner #6 — relokasi section Referal ke rail):** Referal BUKAN bagian kelengkapan Profile — fieldset-nya dipindah dari form ke RAIL (di bawah indikator field belum terisi; ikut sticky di desktop, menumpuk vertikal di rail 340px; mobile: muncul setelah indikator, sebelum form). Tanpa perubahan id/label/perilaku (tetap display-only, di luar body PUT). Hardening test: batas recurse test format-HP dinaikkan 30s→45s (cold-start Vite webkit).
- **2026-09-18 (re-negotiasi owner #5 — grup Info Rekening + komponen shadcn Select):** Urutan grup Rekening menjadi No. Rekening → Pemilik → Bank; input "Bank Lainnya" tetap muncul DI SEBELAH combobox Bank saat "Lainnya" (maxlength 25 + sanitasi server utuh). Dropdown Bank & Hubungan diganti `<select>` native → komponen shadcn Select (reka-ui; komponen baru `app/components/ui/select/` via shadcn-nuxt — side-effect registry terhadap tailwind.css/ package.json DITOLAK: import Google Fonts & dark variant melanggar konvensi brand light-only). Opsi dropdown terurut alfabetis (`localeCompare('id')`); "Lainnya" tetap terakhir sebagai opsi khusus. Test: `isiField` select → klik trigger + opsi by-role dengan verifikasi teks trigger (selectOption tak berlaku untuk komponen non-native); asersi retensi select via teks; gate hidrasi tetap.
- **2026-09-18 (re-negotiasi owner #4 — layout dua postur `/profile-completeness`):** Mobile (<lg) tetap kolom tunggal `max-w-md` (mobile-first PRD utuh); desktop (≥lg) `max-w-5xl` dengan rail kiri sticky 340px (Alert + indikator) + form kanan ber-fieldset grid 2-kolom (pasangan: Alias+No HP, No HP+Hubungan, Bank+Bank Lainnya kondisional, Pemilik+No. Rekening, dua field Referal; Nama/Email/Nama-Darurat lebar penuh; tombol Simpan selebar form). Murni kelas CSS `lg:` — label/id/testid/ARIA/wire/perilaku tak tersentuh (suite E2E 30/30 tetap hijau).
- **2026-09-18 (re-negotiasi owner #3 — validasi & sanitasi form):** Kontrak `shared/domain/profil` diperluas: `sanitasiTeks` (trim+collapse whitespace+buang kontrol), batas panjang (nama bebas 25 / alias 10 / rekening 20), pola nomor (digit+"-"; HP wajib awalan 0 total 9-15 digit), enum dropdown `DAFTAR_BANK` (7+Lainnya) & `DAFTAR_HUBUNGAN` (5), `bankLain` wajib-bersyarat (migrasi 0005 kolom `bank_lain`). PUT: 400 `PROFILE_INVALID` + `details.invalidFields [{field,kode}]` (format/enum) dipisah dari `PROFILE_INCOMPLETE` (kelengkapan); nilai tersimpan = hasil sanitasi. GET + `namaDariGoogle` (prefill AWAL field Nama bila kosong — keputusan owner: cukup nama+email; tidak menimpa data tersimpan, tetap editable). Halaman: `<select>` Bank/Hubungan (placeholder+enum; Lainnya membuka textbox), `maxlength` client-side, inputmode tel/numeric, error format inline per-field (pesan per kode). Test: unit `shared/domain/profil.test.ts` (17); API +3 (enam-pelanggaran PROFILE_INVALID, sanitasi, bankLain bersyarat) + namaDariGoogle + skema bankLain; E2E: generator nilai field-aware (patuh batas), `isiField` selectOption dibungkus recurse verifikasi + gate `tungguHidrasi` (select v-model di-reset hidrasi — penanda `data-terhidrasi` di tombol Simpan), +2 test (error inline HP; alur Lainnya), `skipNetworkMonitoring` untuk 400 terpin.
- **2026-09-18 (re-negotiasi UI owner pasca-review manual — pengelompokan + referal):** Form `/profile-completeness` diubah dari loop 9 input jadi input eksplisit yang dikelompokkan 4 fieldset ber-legend: Pribadi (Nama, Alias, Email, No HP) / Info Kontak Darurat (Nama, No HP, Hubungan) / Info Rekening (Bank, Pemilik, No. Rekening) / Referal (Kode Referal Saya readonly + "Referal Dari" disabled DORMANT sampai Epic 3 — label & kotak disiapkan dulu, permintaan owner). Email dipindah setelah Alias. Label input singkat (keputusan owner); indikator TETAP nama field Lampiran A penuh (UX-DR16 persis). Konsekuensi kontrak: GET `/api/profile` menambah `referralCode` + `usedReferralCode` (repo `bacaReferralOwner`); PUT tak berubah (9 kunci). Selector E2E di-scope `getByRole('group', {name: legend})` karena label singkat duplikat antar grup ("Nama", "No HP"); asersi baru: 4 legend, kode referal terisi disabled, referal-dari disabled, body PUT persis 9 kunci (requestJson intercept).
