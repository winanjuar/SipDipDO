...
- **2026-09-18 (feedback manual — query param dipatenkan + modal tanpa X):** (1) Query param /pendaftaran DIPATENKAN, hanya dua yang sah (whitelist, key lain dibuang via router.replace saat hidrasi): `src=fresh` (rename dari `dari=login` — kedatangan login gagal → presentasi fresh) dan `ref=<kode8>` (DORMANT, dipakai Epic 3, disimpan apa adanya tanpa validasi); (2) modal T&C tanpa tombol X bawaan (`:show-close-button="false"` — kontrak shadcn tak disentuh) — penutupan hanya via Batalkan/Lanjutkan; ESC & klik overlay tetap menutup (keputusan owner). Test baru: whitelist + ketiadaan X.
- **2026-09-18 (feedback manual — rename route/API, res=, alert):** (1) Halaman `pendaftaran.vue` → `register.vue` (route `/register`) dan endpoint ikut: `/api/pendaftaran*` → `/api/register*`; `/status-pendaftaran` TIDAK berubah; (2) query param login dipatenkan hanya `res` (`res=unlinked`/`res=error`; `?error=` otomatis authjs ditranslasi sekali saat hidrasi menjadi `res=error`; key lain dibuang) — produsen redirect unlinked seluruhnya memakai `?res=unlinked`; (3) toast diganti ALERT shadcn (varian delta `success`) di ATAS halaman, auto-hilang 3 detik — composable `useSekaniToast` → `useSekaliAlert` (`useSekaliAlertMasuk` + flag `snd-dash.alert-masuk-berhasil`), dipasang di 4 landing; alert daftar di /status-pendaftaran dari flag `?daftar=berhasil` (mekanisme flag tak berubah); (4) perubahan visual owner di login/register diselaraskan tanpa ubah test (seluruh asersi berbasis testid/copy yang tidak berubah) + aserti baru: alert auto-hide & translasi res=error.
