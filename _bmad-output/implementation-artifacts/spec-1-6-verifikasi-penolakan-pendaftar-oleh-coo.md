...
- **2026-09-23 (prinsip UX + pangkas baris daftar — arahan owner):** halaman Story 1.6 WAJIB mengutamakan UX yang benar-benar nyaman: baris daftar tetap sekali-baca (nama/email truncate satu baris; hint sisaField maks 2 label + "+N lainnya" — isi penuh di halaman Detail), tombol satu baris tanpa wrap, tidak ada jalan buntu (tiap klik berumpan balik), mikrocopy tenang. Route daftar dipindah `pendaftar.vue` → `pendaftar/index.vue` (memperbaiki nested-route Nuxt: tanpa itu klik Detail tak berpindah halaman).
...
- **2026-09-23 (renegotiasi owner pasca-merge #2 — detail data pendaftar):** baris `/pendaftar` menambah tombol **Detail** → **HALAMAN penuh `/pendaftar/:id`** (bukan pop-up — revisi owner 2026-09-23 sore) yang MENIRU layout halaman kelengkapan `/profile-completeness` (fieldset ber-legend sama: Profil Pemilik / Info Kontak Darurat / Info Rekening; input read-only gaya muted-disabled; urutan & label PERSIS), read-only via endpoint BARU `GET /api/pendaftar/:id` (gate COO; hanya baris `diajukan` → selain itu 404; id tak sah → 400; bentuk field PERSIS wire `GET /api/profile`). Halaman detail memuat aksi **Verifikasi** (calon lengkap; dialog konfirmasi dua-langkah yang sama; sukses/race → kembali ke daftar). Urutan tombol di baris daftar dipin: **Verifikasi (kiri) — Detail (tengah, link halaman) — Tolak (ujung kanan, merah/destructive)**, satu baris tanpa wrap. Alasan: COO harus bisa MEMERIKSA isi persis seperti diisi calon, tanpa nambah effort bolak-balik (verifikasi tersedia di halaman detail).
